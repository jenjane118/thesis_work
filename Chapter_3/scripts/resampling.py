###adapted version to include lower tail p-values in output
...
############# add columns 2tp-value and lp-value to catch 2t and lower
############# adj p-value should be using up-val
columns = ["Orf","Name","Desc","Sites","Mean Ctrl","Mean Exp","log2FC", "Sum Ctrl", "Sum Exp", "Delta Mean","2tp-value","pval-utail","pval-ltail","Adj. p-value"]
...
######################## added ltail and utail to list for row
            (orf, name, desc, n, mean1, mean2, sum1, sum2, test_obs, log2FC, pval_2tail, pval_utail, pval_ltail) = row
            if self.Z==True:
...
################## added entries here for pval_utail and pval_ltail columns
            else: self.output.write("%s\t%s\t%s\t%d\t%1.1f\t%1.1f\t%1.2f\t%1.1f\t%1.2f\t%1.1f\t%1.5f\t%1.5f\t%1.5f\t%1.5f\n" % (orf, name, desc, n, mean1, mean2, log2FC, sum1, sum2, test_obs, pval_2tail, pval_utail, pval_ltail, qval[i]))
        self.output.close()
...
####### inserted lower and upper tail here ###############################
